...
import explodesvgMetadata from './explodesvg/block.json';
import ExplodesvgEdit from './explodesvg/edit';
import ExplodesvgSave from './explodesvg/save';
import './explodesvg/style.scss';

registerBlockType(explodesvgMetadata.name, {
	...explodesvgMetadata,
	edit: ExplodesvgEdit,
	save: ExplodesvgSave,
});
...
const { PanelBody, TextControl, SelectControl, ColorPalette, RangeControl, Button, Modal, ToggleControl } = wp.components;
...
function addAttributes(settings, name) {
    // Ensure attributes are added to all blocks
    if (true) {
            ...
            <Fragment>
                ...
                <InspectorControls>
                    <PanelBody title={__('Page Transition Effect', 'wp-logo-explode')} initialOpen={false}>
                        <ToggleControl
                            label={__('Link Icon Grid Tile', 'wp-logo-explode')}
                            help={linkIconGridTile ? __('This block is linked to an Icon Grid tile. Hover triggers tile highlight, click triggers tile link.', 'wp-logo-explode') : __('Enable to link this block to an Icon Grid tile.', 'wp-logo-explode')}
                            checked={linkIconGridTile}
                            onChange={(value) => setAttributes({ linkIconGridTile: value })}
                        />

                        <TextControl
                            label={__('Transition ID', 'wp-logo-explode')}
                            help={linkIconGridTile
                                ? __('Must match the Transition ID of an Icon Grid tile.', 'wp-logo-explode')
                                : __('Unique ID linking the start and end logos (e.g. "my-logo").', 'wp-logo-explode')}
                            value={transitionId}
                        ...
                        {linkIconGridTile ? (
                            <RangeControl
                                label={__('Icon Grid Instance', 'wp-logo-explode')}
                                help={__('Which Icon Grid instance to target (1 = first on page, 2 = second, etc.)', 'wp-logo-explode')}
                                value={iconGridInstance || 1}
                                onChange={(value) => setAttributes({ iconGridInstance: value })}
                                min={1}
                                max={10}
                            />
                        ) : (
                            <Fragment>
                                ...
                                <TextControl
                                    label={__('Animate Element ID', 'wp-logo-explode')}
                                    help={__('ID of an external element to animate (anywhere on the page). Takes priority over Animate Selector.', 'wp-logo-explode')}
                                    value={transitionAnimateDomId}
                                    onChange={(value) => setAttributes({ transitionAnimateDomId: value })}
                                    placeholder="my-external-element-id"
                                />
                            </Fragment>
                        )}
...
function addSaveProps(extraProps, blockType, attributes) {
    ...
    // Handle Link Icon Grid Tile mode
    if (linkIconGridTile && transitionId) {
        extraProps['data-link-icon-grid'] = 'true';
        extraProps['data-transition-id'] = transitionId;
        extraProps['data-icon-grid-instance'] = iconGridInstance || 1;
    ...
    // Handle regular transition mode
    else if (transitionId && transitionRole) {
        extraProps['data-transition-id'] = transitionId;
        ...
        if (transitionAnimateDomId) extraProps['data-transition-animate-dom-id'] = transitionAnimateDomId;
        if (durationExpand) extraProps['data-transition-duration-expand'] = durationExpand;
        if (durationShrink) extraProps['data-transition-duration-shrink'] = durationShrink;
        if (scaleExplode) extraProps['data-transition-scale-explode'] = scaleExplode;
